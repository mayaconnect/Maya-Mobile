/**
 * Maya Connect V2 — Store Management Screen (StoreOperator)
 *
 * Dark theme. Manager-only. Capabilities:
 *  • Store image upload / delete
 *  • Opening hours editor + save
 *  • Phone number edit + save
 *  • Read-only informations
 *
 * Accepts storeId via route params or falls back to activeStore.
 */
...
export default function StoreManagementScreen() {
  ...
  const activeStoreInfo = operatorStores.find((s) => s.id === storeId);
  ...
  const saveHoursMutation = useMutation({
    mutationFn: async (hours: StoreOpeningHours) => {
      ...
      console.log('[StoreManagement-Op] PATCH opening hours:', JSON.stringify(dto));
    ...
    onError: (err: any) => {
      ...
      console.error('[StoreManagement-Op] PATCH hours error:', err?.response?.status, err?.response?.data, detail);
      alert('Erreur', err?.response?.status === 403
        ? "Vous n'avez pas les droits pour modifier ce magasin. Un rôle Manager est requis."
  ...
  const savePhoneMutation = useMutation({
    mutationFn: async (phone: string) => {
      ...
      console.log('[StoreManagement-Op] PATCH phone:', JSON.stringify(dto));
      return storesApi.patchStore(storeId, dto);
    },
    onSuccess: () => {
    ...
    onError: (err: any) => {
      const detail = extractApiError(err);
      console.error('[StoreManagement-Op] PATCH phone error:', err?.response?.status, err?.response?.data, detail);
      alert('Erreur', err?.response?.status === 403
      ...
      <View style={styles.bg}>
        ...
        <View style={styles.centerContent}>
          <View style={styles.lockIconWrap}>
            <Ionicons name="lock-closed" size={wp(32)} color="rgba(255,255,255,0.4)" />
          </View>
          <Text style={styles.fallbackTitle}>Accès restreint</Text>
          <Text style={styles.fallbackText}>
            Seuls les opérateurs avec le rôle Manager peuvent gérer les informations du magasin.
          </Text>
  ...
  if (storeQ.isError || !store) {
    return (
      <View style={styles.bg}>
        <ErrorState fullScreen title="Erreur" description="Impossible de charger les données du magasin." onRetry={() => storeQ.refetch()} />
      </View>
    );
  }
      ...
      >
        <View style={styles.headerRow}>
          ...
          <View style={[styles.managerBadge]}>
            <Ionicons name="shield-checkmark" size={wp(12)} color="#4ADE80" />
          ...
          {editingPhone ? (
            <View style={styles.phoneEditWrap}>
              ...
              <View style={styles.phoneActions}>
                ...
                <TouchableOpacity
                  style={[styles.savePhoneBtn, !phoneValue.trim() && styles.saveBtnDisabled]}
                  onPress={() => savePhoneMutation.mutate(phoneValue.trim())}
                  disabled={!phoneValue.trim() || savePhoneMutation.isPending}
                >
            ...
            <View style={styles.phoneDisplayWrap}>
              ...
              {!store.phone && (
                <Text style={styles.phoneHint}>Appuyez sur l'icône crayon pour ajouter</Text>
              )}
            </View>
          )}
        </View>
...
const styles = StyleSheet.create({
  ...
  managerBadgeText: { fontSize: wp(10), fontFamily: fontFamily.semiBold, color: '#4ADE80' },
  ...
  saveBtnDisabled: { backgroundColor: 'rgba(255,106,0,0.35)' },
  ...
  fallbackText: { ...textStyles.body, color: 'rgba(255,255,255,0.25)', textAlign: 'center', lineHeight: 22 },
  backLinkBtn: {
    marginTop: spacing[3],
    backgroundColor: colors.orange[500],

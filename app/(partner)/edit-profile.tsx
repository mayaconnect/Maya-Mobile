/**
 * Maya Connect V2 — Edit Profile Screen (Partner)
 */
...
function translateError(field: string, msg: string): string {
  ...
  return msg
    ...
    .replace(/^The\s+field\s+\w+\s+is\s+required\.?$/i, `${fieldFr} est requis.`)
    .replace(/is required/gi, 'est requis')
    .replace(/is invalid/gi, 'est invalide')
    .replace(/must be/gi, 'doit être')
    .replace(/must not be/gi, 'ne doit pas être')
    .replace(/is too long/gi, 'est trop long')
    .replace(/is too short/gi, 'est trop court')
    .replace(/maximum length/gi, 'longueur maximale')
    .replace(/minimum length/gi, 'longueur minimale')
    .replace(/already exists/gi, 'existe déjà')
    .replace(/not found/gi, 'introuvable')
    .replace(/invalid format/gi, 'format invalide')
...
function Field({
  label, value, onChangeText, onBlur, placeholder, icon,
  error, keyboardType, autoCapitalize, readonly, rightElement,
}: FieldProps) {
  ...
  return (
    <TouchableOpacity
      ...
      style={[
        ...
        !!error && fStyles.wrapError,
        readonly && fStyles.wrapReadonly,
      ]}
    >
      {icon && (
      ...
      <View style={fStyles.inner}>
        <Text style={[fStyles.label, focused && fStyles.labelFocused, !!error && fStyles.labelError]}>
          {label}
        </Text>
        {readonly ? (
...
export default function PartnerEditProfileScreen() {
  ...
  const fullName = [user?.firstName, user?.lastName].filter(Boolean).join(' ') || 'Partenaire';
    ...
    onError: (err: any) => {
      ...
      if (validationErrors) {
        const lines = Object.entries(validationErrors)
          .flatMap(([field, msgs]) =>
            (msgs as string[]).map((m) => translateError(field, m))
          )
          .filter(Boolean);
    ...
    >
      ...
      >
        ...
        <View style={styles.topRow}>
          <TouchableOpacity style={styles.backBtn} onPress={() => router.push('/(partner)/profile' as any)} activeOpacity={0.8}>
        ...
        <View style={styles.row}>
          <View style={{ flex: 1 }}>
            <Controller
              ...
              render={({ field: { onChange, onBlur, value } }) => (
                <>
                  <Field label="Prénom" value={value} onChangeText={onChange} onBlur={onBlur}
          ...
          <View style={{ flex: 1 }}>
            <Controller
              ...
              render={({ field: { onChange, onBlur, value } }) => (
                <>
                  <Field label="Nom" value={value} onChangeText={onChange} onBlur={onBlur}
                    icon="person-outline" placeholder="Dupont" autoCapitalize="words"
        ...
        <Controller
          ...
          render={({ field: { onChange, onBlur, value } }) => (
            <>
              <Field label="Téléphone" value={value ?? ''} onChangeText={onChange} onBlur={onBlur}
                icon="call-outline" placeholder="+33 6 00 00 00 00"
                keyboardType="phone-pad" error={errors.phoneNumber?.message}
              />
        ...
        <Controller
          ...
          render={({ field: { onChange, onBlur, value } }) => (
            <>
              <Field label="Rue" value={value ?? ''} onChangeText={onChange} onBlur={onBlur}
                icon="map-outline" placeholder="12 rue de la Paix" autoCapitalize="words"
        ...
        <View style={styles.row}>
          <View style={{ flex: 0.42 }}>
            <Controller
              ...
              render={({ field: { onChange, onBlur, value } }) => (
                <>
                  <Field label="Code postal" value={value ?? ''} onChangeText={onChange} onBlur={onBlur}
                    placeholder="75001" keyboardType="numeric" error={errors.postalCode?.message}
                  />
          ...
          <View style={{ flex: 1 }}>
            <Controller
              ...
              render={({ field: { onChange, onBlur, value } }) => (
                <>
                  <Field label="Ville" value={value ?? ''} onChangeText={onChange} onBlur={onBlur}
                    placeholder="Paris" autoCapitalize="words" error={errors.city?.message}
                  />
        ...
        <View style={styles.row}>
          <View style={{ flex: 1 }}>
            <Controller
              ...
              name="state"
              render={({ field: { onChange, onBlur, value } }) => (
                <Field label="Région" value={value ?? ''} onChangeText={onChange} onBlur={onBlur}
                  placeholder="Île-de-France" autoCapitalize="words"
                />
              )}
            />
          </View>
          <View style={{ flex: 1 }}>
            <Controller
              ...
              name="country"
              render={({ field: { onChange, onBlur, value } }) => (
                <Field label="Pays" value={value ?? ''} onChangeText={onChange} onBlur={onBlur}
                  placeholder="France" autoCapitalize="words"
                />
              )}
            />
          </View>
        ...
        <View style={styles.actions}>
          ...
          <TouchableOpacity style={styles.cancelBtn} onPress={() => router.push('/(partner)/profile' as any)}>
      ...
      <Modal visible={successModal} transparent animationType="fade" statusBarTranslucent
        onRequestClose={() => { setSuccessModal(false); router.push('/(partner)/profile' as any); }}>
        <Pressable style={mStyles.backdrop} onPress={() => { setSuccessModal(false); router.push('/(partner)/profile' as any); }} />
        <View style={mStyles.sheet}>
          ...
          <MButton title="Super !" onPress={() => { setSuccessModal(false); router.push('/(partner)/profile' as any); }} style={mStyles.btn} />
        </View>
      </Modal>
